...
export const Signup: React.FC = () => {
  ...
  return (
    <div className="min-h-screen text-white flex flex-col items-center justify-center p-6 relative overflow-hidden"
      style={{ background: 'radial-gradient(ellipse at 70% 20%, rgba(192,132,252,0.10) 0%, transparent 55%), radial-gradient(ellipse at 25% 80%, rgba(255,107,157,0.10) 0%, transparent 55%), #0A0612' }}>
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3 mb-1">
          <div className="w-14 h-14 rounded-3xl flex items-center justify-center text-3xl"
            style={{ background: 'linear-gradient(135deg, #C084FC, #FF6B9D)', boxShadow: '0 8px 32px rgba(192,132,252,0.4)' }}>
            🌸
          </div>
          <div className="text-center">
            <p className="font-display font-extrabold text-[22px] tracking-widest leading-none"
              style={{ background: 'linear-gradient(135deg, #C084FC, #FF6B9D)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              KAWAI SAKURA
            </p>
            <p className="text-[13px] font-medium mt-1.5" style={{ color: 'rgba(255,255,255,0.35)' }}>Create your account 🌸</p>
          </div>
        </div>

        {error && (
          <div className="flex items-center gap-2 p-3 rounded-2xl text-[13px]"
        ...
        <motion.button whileTap={{ scale: 0.97 }} onClick={handleGoogle} disabled={loading}
          className="flex items-center justify-center gap-3 w-full font-bold py-3.5 rounded-2xl text-[14px]"
        ...
        <form onSubmit={handleSignup} className="flex flex-col gap-3.5">
          {[
            { label: 'YOUR NAME', type: 'text', value: name, set: setName, placeholder: 'Sakura Chan' },
            { label: 'EMAIL', type: 'email', value: email, set: setEmail, placeholder: 'your@email.com' },
          ].map(({ label, type, value, set, placeholder }) => (
            <div key={label} className="flex flex-col gap-1.5">
              <label className="text-[12px] font-bold tracking-wide" style={{ color: 'rgba(255,255,255,0.5)' }}>{label}</label>
          ...
          <motion.button type="submit" disabled={loading} whileTap={{ scale: 0.97 }}
            className="w-full font-bold py-3.5 rounded-2xl text-[14px] text-black mt-1 flex items-center justify-center"
            style={{ background: 'linear-gradient(135deg, #C084FC, #FF6B9D)', boxShadow: loading ? 'none' : '0 6px 20px rgba(192,132,252,0.35)' }}>
            {loading ? <Loader2 size={18} className="animate-spin text-white" /> : '🌸 Create Account'}
          </motion.button>
        </form>

        <p className="text-center text-[13px]" style={{ color: 'rgba(255,255,255,0.35)' }}>
          Already have an account?{' '}
          <Link to="/login" className="font-bold" style={{ color: '#C084FC' }}>Log in</Link>
        </p>

        <button onClick={() => navigate('/')} className="text-[12px] text-center" style={{ color: 'rgba(255,255,255,0.25)' }}>
          ← Back to app
        </button>
      </motion.div>
    </div>
